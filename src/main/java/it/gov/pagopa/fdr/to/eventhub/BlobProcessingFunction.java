...
public class BlobProcessingFunction {
  ...
  public synchronized void processFDR1BlobFiles(
    ...
    if (!CommonUtil.validateBlobMetadata(blobMetadata)) {
        logger.warning(String.format("[FDR1] Skipping processing for Blob container: %s, name: %s, size in bytes: %d",
    ...
    logger.fine(String.format("[FDR1] Triggered at: %s for Blob container: %s, name: %s, size in bytes: %d",
    ...
    try (InputStream decompressedStream =
      ...
      logger.fine(String.format("[FDR1] Parsed Finished at: %s for Blob container: %s, name: %s, size in bytes: %d",
                      LocalDateTime.now().format(DateTimeFormatter.ofPattern(CommonUtil.LOG_DATETIME_PATTERN)),
                      fdr1Container,
                      blobName,
                      content.length));
      ...
      logger.fine(String.format("[FDR1] Execution Finished at: %s for Blob container: %s, name: %s, size in bytes: %d",
                      LocalDateTime.now().format(DateTimeFormatter.ofPattern(CommonUtil.LOG_DATETIME_PATTERN)),
                      fdr1Container,
                      blobName,
                      content.length));
    } catch (Exception e) {
      String exceptionDetails = getExceptionDetails(ErrorCodes.FDR1_E1.getCode(), fdr1Container, blobName, blobMetadata.get(SESSION_ID_METADATA_KEY), retryIndex);
  ...
  public void processFDR3BlobFiles(
    ...
    try (InputStream decompressedStream =
      ...
      String exceptionDetails = getExceptionDetails(ErrorCodes.FDR3_E1.getCode(), fdr3Container, blobName, blobMetadata.get(SESSION_ID_METADATA_KEY), retryIndex);
      throw new AlertAppException(e.getMessage(), e.getCause(), exceptionDetails);
    }
  }
}
