...
public class CommonUtil {
	...
	public static HttpResponseMessage checkBlobMetadata (HttpRequestMessage<Optional<String>> request, Map<String, String> blobMetadata, String fileName, String container) {
	...
	public static BlobFileData getBlobFile(
		...
		try {
			...
			return new BlobFileData(blobName, outputStream.toByteArray(), metadata);
	...
	public static List<BlobFileData> getBlobFilesInDateRange(HttpRequestMessage<Optional<String>> request,
			String storageEnvVar, String containerName, LocalDateTime from, LocalDateTime to, ExecutionContext context) {
		try {
			BlobContainerClient containerClient =
					blobServiceClientWrapper.getBlobContainerClient(storageEnvVar, containerName);
			...
			// Convert to OffsetDateTime (Azure SDK required)
			OffsetDateTime fromOffset = from != null ? from.atOffset(OffsetDateTime.now().getOffset()) : null;
			OffsetDateTime toOffset = to != null ? to.atOffset(OffsetDateTime.now().getOffset()) : null;

			for (BlobItem blobItem : containerClient.listBlobs()) {
				OffsetDateTime lastModified = blobItem.getProperties().getLastModified();
				if (lastModified != null &&
						(fromOffset == null || !lastModified.isBefore(fromOffset)) &&
						(toOffset == null || !lastModified.isAfter(toOffset))) {
					BlobClient blobClient = containerClient.getBlobClient(blobItem.getName());
					ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
					blobClient.downloadStream(outputStream);
					Map<String, String> metadata = Optional.ofNullable(blobItem.getMetadata()).orElse(new HashMap<>());
					CommonUtil.checkBlobMetadata(request, metadata, blobItem.getName(), containerName);
					BlobFileData blobFileData = new BlobFileData(blobItem.getName(), outputStream.toByteArray(), metadata);
	...
	public static HttpResponseMessage unprocessableEntity(HttpRequestMessage<?> request, String message) {
		return response(request, HttpStatus.UNPROCESSABLE_ENTITY, message);
	}

	public static HttpResponseMessage serviceUnavailable(HttpRequestMessage<?> request, String message) {
