...
class HttpMassiveBlobRecoveryFunctionTest {
	...
	void testMissingMetadata() throws Exception {
		...
		String requestBody =
				objectMapper.writeValueAsString(
						Map.of("fileName", "test.xml", "container", "fdr1-flows"));
		...
		try (MockedStatic<CommonUtil> mockedUtil = mockStatic(CommonUtil.class)) {
			...
			mockedUtil.when(() -> CommonUtil.unprocessableEntity(any(HttpRequestMessage.class), anyString())).thenReturn(mockResponse);
	...
	void testFDR1FilenameRequestOK() throws Exception {
		...
		try (MockedStatic<CommonUtil> mockedUtil = mockStatic(CommonUtil.class)) {
			...
			.thenReturn(true);
			mockedUtil.when(() -> CommonUtil.ok(any(HttpRequestMessage.class), anyString())).thenReturn(mockResponse);
	...
	void testFDR1DateRangeRequestOK() throws Exception {
		...
		try (MockedStatic<CommonUtil> mockedUtil = mockStatic(CommonUtil.class)) {
			mockedUtil
			.when(() -> CommonUtil.getBlobFilesInDateRange(anyString(), anyString(), anyString(), any(LocalDate.class), any(LocalDate.class), any(ExecutionContext.class)))
	...
	void testFDR3FilenameRequestOK() throws Exception {
		...
		String requestBody =
				objectMapper.writeValueAsString(
						Map.of("fileName", "test.json", "container", "fdr3-flows"));
		when(mockRequest.getBody()).thenReturn(Optional.of(requestBody));
		...
		try (MockedStatic<CommonUtil> mockedUtil = mockStatic(CommonUtil.class)) {
			...
			.when(
					...
					CommonUtil.processXmlBlobAndSendToEventHub(
							any(), any(), any(), any(), anyBoolean(), anyBoolean()))
			.thenReturn(false);
			mockedUtil.when(() -> CommonUtil.multiStatus(any(HttpRequestMessage.class), anyString())).thenReturn(mockResponse);
